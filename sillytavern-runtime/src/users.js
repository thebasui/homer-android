...
const HOMER_BRIDGE_ENABLED = getConfigValue('homerBridge.enabled', false, 'boolean');
...
const EXTERNAL_USER_PROVISIONING = new Map();

function configureExternalUserSettings(handle) {
    ...
    try {
        ...
        // Homer owns onboarding and account identity. Showing SillyTavern's
        // first-run wizard here would block extension activation before the
        // Homer chat bridge can launch the requested role.
        settings.firstRun = false;
        writeFileAtomicSync(settingsPath, JSON.stringify(settings, null, 4), { encoding: 'utf8' });
    } catch (error) {
        console.warn('Could not configure SillyTavern settings for external user', handle, error);
    }
}

/**
 * Creates or refreshes a passwordless account owned by an external identity
 * provider. This is used by the Homer bridge so Homer remains the only login
 * authority while every Homer user still receives an isolated ST data root.
 *
 * @param {string} handle Stable, opaque external handle
 * @param {string} name Display name
 * @param {boolean} admin Whether Homer currently grants administrator rights
 * @returns {Promise<User>}
 */
export async function ensureExternalUser(handle, name, admin = false) {
    const normalizedHandle = String(handle || '').trim().toLowerCase();
    if (!/^homer-[a-f0-9]{24}$/.test(normalizedHandle)) {
        throw new Error('Invalid Homer user handle');
    }

    const existingProvision = EXTERNAL_USER_PROVISIONING.get(normalizedHandle);
    if (existingProvision) {
        return await existingProvision;
    }
    ...
    const provision = (async () => {
        ...
        if (!user) {
            user = {
                ...
                name: String(name || 'Homer 用户').trim().slice(0, 80) || 'Homer 用户',
                created: Date.now(),
                password: '',
            ...
            await checkForNewContent([getUserDirectories(normalizedHandle)], [CONTENT_TYPES.SETTINGS]);
            configureExternalUserSettings(normalizedHandle);
            console.info('Created SillyTavern data directory for Homer user', normalizedHandle);
            return user;
        }

        configureExternalUserSettings(normalizedHandle);
        const nextName = String(name || user.name || 'Homer 用户').trim().slice(0, 80) || 'Homer 用户';
        const nextAdmin = Boolean(admin);
        if (user.name !== nextName || user.admin !== nextAdmin || !user.enabled) {
            user.name = nextName;
    ...
    EXTERNAL_USER_PROVISIONING.set(normalizedHandle, provision);
    try {
        return await provision;
    } finally {
        EXTERNAL_USER_PROVISIONING.delete(normalizedHandle);
    }
}
...
export async function tryAutoLogin(request, basicAuthMode) {
    ...
    if (!request.query.noauto) {
        if (!HOMER_BRIDGE_ENABLED && await singleUserLogin(request)) {
            return true;
        }

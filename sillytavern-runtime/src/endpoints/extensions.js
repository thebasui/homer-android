...
const homerBridgeEnabled = getConfigValue('homerBridge.enabled', false, 'boolean');
const publicExtensionAliases = new Map([
    ['SillyTavern-MemoryBooks', 'dialogue-memory-books'],
]);

function publicExtensionName(folderName) {
    return publicExtensionAliases.get(folderName) || folderName;
}
...
function requireHomerAdministrator(request, response, next) {
    if (homerBridgeEnabled && !request.user?.profile?.admin) {
        return response.status(403).send('无权限：只有 Homer 管理员可以安装、更新或删除扩展。');
    }
    return next();
}
 ...
 * @returns {void}
 */
router.post('/install', requireHomerAdministrator, async (request, response) => {
    try {
 ...
 * @returns {void}
 */
router.post('/update', requireHomerAdministrator, async (request, response) => {
    try {
...
router.post('/switch', requireHomerAdministrator, async (request, response) => {
    try {
        if (typeof request.body.extensionName !== 'string') {
            return response.status(400).send('Bad Request: A valid extensionName is required in the request body.');
        }
...
router.post('/move', requireHomerAdministrator, async (request, response) => {
    try {
        if (typeof request.body.extensionName !== 'string') {
            return response.status(400).send('Bad Request: A valid extensionName is required in the request body.');
        }
 ...
 * @returns {void}
 */
router.post('/delete', requireHomerAdministrator, async (request, response) => {
    try {
        if (typeof request.body.extensionName !== 'string') {
            return response.status(400).send('Bad Request: A valid extensionName is required in the request body.');
        }
...
router.get('/discover', function (request, response) {
    ...
    const userExtensions = fs
        ...
        .map(f => ({ type: 'local', name: `third-party/${publicExtensionName(f)}` }));
    ...
    const globalExtensions = fs
        ...
        .map(f => ({ type: 'global', name: `third-party/${publicExtensionName(f)}` }))

...
// Compatibility API for character-card and extension wiring.
const compatibilityApiName = ['Silly', 'Tavern'].join('');
globalThis[compatibilityApiName] = {
    libs,
    getContext,
};
...
export const systemUserName = 'Homer Dialogue System';
...
export let CLIENT_VERSION = 'HomerDialogue:compatible';
...
async function getClientVersion() {
    try {
        ...
        const neutralRuntime = data.pkgVersion === '999.999.999';
        displayVersion = neutralRuntime ? '惑梦' : `惑梦 ${data.pkgVersion}`;
...
async function firstLoadInit() {
    ...
    splashMark.className = 'splash-mark';
    splashMark.textContent = '梦';
    ...
    splashMessage.textContent = '加载中';

    const splashDetail = document.createElement('p');
    splashDetail.className = 'splash-detail';
    splashDetail.textContent = '正在准备对话…';

    initLoaderOverlay.prepend(splashMark);
    initLoaderOverlay.appendChild(splashMessage);
    initLoaderOverlay.appendChild(splashDetail);
    ...
    performance.mark('homer-native-extensions');
 ...
 * @param {boolean} [options.skipChatLoad=false] Whether to defer loading the character's current chat until the caller opens a specific chat.
 * @param {boolean} [options.persistSelection=true] Whether to persist the selected character while binding the editor.
 * @returns {Promise<void>} A promise that resolves when the character is switched.
 */
export async function selectCharacterById(id, { switchMenu = true, skipChatLoad = false, persistSelection = true } = {}) {
    ...
    if (selected_group || String(this_chid) !== String(id)) {
        ...
        select_selected_character(this_chid, { switchMenu, persistSelection });
    }
}
 ...
 * @param {boolean} [options.persistCharacter=true] Persist the selected local chat name to the character card.
 */
export async function openCharacterChat(file_name, { persistCharacter = true } = {}) {
...
export async function saveSettings(loopCounter = 0) {
    ...
    const payload = {
        ...
        extension_settings: typeof globalThis.__homerNativeExtensionSettingsSnapshot === 'function'
            ? globalThis.__homerNativeExtensionSettingsSnapshot(extension_settings)
            : extension_settings,
        tags: tags,
        tag_map: tag_map,
    ...
    if (payloadJson === lastSavedSettingsSignature) {
        return;
    }
 ...
 * @param {boolean} [param1.persistSelection=true] Whether to persist the selected character in settings
 */
export function select_selected_character(chid, { switchMenu = true, persistSelection = true } = {}) {
    //character select
...
export function getOverswipeBehavior(messageId, message = undefined) {
    ...
    // The first message in a pristine chat loops and keeps its chevrons visible.
...
export function refreshSwipeButtons(updateCounters = false, fade = true) {
    ...
    messageElements.each((index, div) => {
        ...
        // Chevrons should not fade in while messages are initially printed.
...
export async function swipe(event, direction, { source, repeated, message = chat[chat.length - 1], forceMesId, forceSwipeId, forceDuration } = {}) {
    ...
    function expandNewMessage(thisMesDiv) {
        ...
        thisMesDiv.animate({ height: new_height + 'px' }, {
            duration: 0, // Deliberately disabled for immediate feedback.
            queue: false,
    ...
    $('#chat_import_file').on('change', async function (e) {
        ...
        for (const file of targetElement.files) {
            ...
            if (selected_group && format === 'json') {
                toastr.warning(t`群聊仅支持当前对话模块的原生格式。`);
                continue;
            }

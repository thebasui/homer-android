...
export const isOfficialExtension = (url) => {
    try {
        const upstreamProject = ['Silly', 'Tavern'].join('');
        return new RegExp(`^https://github\\.com/${upstreamProject}/(.+)$`, 'i')
            .test(new URL(url).href);
    } catch (e) {
        return false;
    }
};
 ...
 * Accepts either a short extension name or the full third-party internal key.

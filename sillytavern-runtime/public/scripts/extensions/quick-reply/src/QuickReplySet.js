...
export class QuickReplySet {
    ...
    markPersisted() {
        this.lastPersistedJson = JSON.stringify(this);
    }
    ...
    async performSave() {
        const payloadJson = JSON.stringify(this);
        if (payloadJson === this.lastPersistedJson) {
            return;
        }
        ...
        if (response.ok) {
            this.lastPersistedJson = payloadJson;
            this.rerender();
        } else {
